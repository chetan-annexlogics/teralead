...
Vtiger.Class("VTDevKBView_Js",{
        ...
        app.request.post({data:params}).then(
            function(err,data){
                if(err==null){
                    ...
                    var thisinstance = new VTDevKBView_Js();
    ...
    if(viewTarget == 'List'){
        ...
        if(targetModule != 'VTDevKBView'){
            var objParams = {
                'module':'VTDevKBView',
                'action':'ActionAjax',
                'mode':'checkVTDevKBViewEnable',
                'source_module':source_module
            };
            app.request.post({data:objParams}).then(
                function(err,data){
                    console.log("appnav .navbar-nav")
                    if(err == null && data.isEnable == true){
                        jQuery('#appnav .navbar-nav').prepend(VTDevKBView_Js.kanbanButtonHtml);
                            url = 'index.php?module=VTDevKBView&view=Index';
        ...
        var url = 'index.php?module=VTDevKBView&view=Index';
        url +='&viewname='+cvid;
        url +='&source_module='+source_module;
        jQuery('#appnav .navbar-nav').prepend(VTDevKBView_Js.kanbanButtonHtml);
        if(url == undefined){
     ...
     if(viewTarget == 'List'){
         if(targetModule != 'VTDevKBView'){
             ...
             app.request.post({data:objParams}).then(
                 function(err,data){
                     if(err == null && data.isEnable == true){
                         jQuery('#appnav .navbar-nav').prepend(VTDevKBView_Js.kanbanButtonHtml);
                         registerEventsClickVTDevKBButton(source_module);
                     }
                 }
             );
         }
         ...
         url +='&source_module='+source_module;
         jQuery('#appnav .navbar-nav').prepend(VTDevKBView_Js.kanbanButtonHtml);
         registerEventsClickVTDevKBButton(source_module,url);
     }
}
    ...
    getOtherField:function(){
        var items = [];
        jQuery('#VTDevKBConfigure .listOtherField tbody').find('select.selectedOtherField').each(function(index, el){
    ...
    registerEventForSelectPrimaryField: function(){
        var thisInstance = this;
        var elementPrimaryField = jQuery('#VTDevKBConfigure select[name="primaryField"]');
    ...
    registerEventForSaveButton:function(){
        var thisInstance = this;
        jQuery('#save_vtdevkbview_setting').on('click',function(){
            var form = jQuery('#VTDevKBConfigure');
            var isDefaultPage = form.find('[name="isDefaultPage"]').prop('checked') ? 1:0;
            var params = {
                'module':'VTDevKBView',
                'action':'SaveAjax',
                'mode': 'saveVTDevKBViewSetting',
                'source_module':thisInstance.source_module,
            ...
            app.request.post({data:params}).then(
                function(err,data){
                    if(err == null){
                        ...
                        if(thisInstance.targetModule == "VTDevKBView"){
