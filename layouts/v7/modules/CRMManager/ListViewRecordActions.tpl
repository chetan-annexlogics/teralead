...
<div class="table-actions">
    ...
    <span class="more dropdown action">
        ...
        <ul class="dropdown-menu">
            {if $LISTVIEW_ENTRY->get('cmmanager_cf_959') eq "Yes"}
                <li><a data-id="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);" class="updateChildCrm">{vtranslate('Update child CRM', $MODULE)}</a></li>
            {else}
                  <li><a data-id="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);" class="managerChildCrm">{vtranslate('Create child CRM', $MODULE)}</a></li>
            {/if}
            <li><a data-id="{$LISTVIEW_ENTRY->getId()}" href="{$LISTVIEW_ENTRY->get('cmmanager_cf_945')}" class="gotoChildCrm" target="_blank">{vtranslate('Goto child CRM', $MODULE)}</a></li>
            <li><a data-id="{$LISTVIEW_ENTRY->getId()}" href="javascript:void(0);" class="pushDataToChildCrm">{vtranslate('Push data to child CRM', $MODULE)}</a></li>
