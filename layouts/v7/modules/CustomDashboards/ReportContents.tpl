...
{strip}
    ...
    <div id="reportDetails" class="contents-bottomscroll" style="{if $WIDGET_COLOR}background-color:{$WIDGET_COLOR}{/if}">
        <div class="bottomscroll-div">
            <input type="hidden" id="updatedCount" value="{$NEW_COUNT}" />
            {if $DATA neq ''}
{*                {assign var=HEADERS value=$DATA[0]}*}
               {assign var=HEADERS value=$DATA[0]}
                <table class="table table-bordered" style="text-align: left;{if $WIDGET_COLOR}background-color:{$WIDGET_COLOR}{/if}">
                    <thead>
                        <tr class="blockHeader">
                                ...
                                {foreach from=$VALUES item=VALUE key=NAME}
                                    {if ($NAME eq $FIRST_FIELD || $NAME|strstr:{$FIRST_FIELD}) && ($FIRST_VALUE eq $VALUE || $FIRST_VALUE eq " ")}
                                        {if $FIRST_VALUE eq " " || $VALUE eq "-"}
                                              <td style="word-break: break-word;{if $WIDGET_TEXT_COLOR neq ''}color:{$WIDGET_TEXT_COLOR} !important{/if}" class="summary" >{$VALUE}</td>
                                        {else}
                                    ...
                                    {else if ($NAME eq $THIRD_FIELD || $NAME|strstr:$THIRD_FIELD) && ($THIRD_VALUE eq $VALUE || $THIRD_VALUE eq " ")}
                                        ...
                                        {/if}
                                    {else}
                                        <td style="border-bottom: 0;{if $WIDGET_TEXT_COLOR neq ''}color:{$WIDGET_TEXT_COLOR} !important{/if}">{$VALUE}</td>
                            ...
                            <tr>
                                {foreach from=$VALUES item=VALUE key=NAME}
                                    {foreach from=$HEADERS item=HEADER}
                                        ...
                                        {/if}
                                    {/foreach}
                                    <td style="word-break: break-word;{if $WIDGET_TEXT_COLOR neq ''}color:{$WIDGET_TEXT_COLOR} !important{/if}" class="{$HEADER_FIELD_NAME}" field-value="{$VALUE}" >{$VALUE}</td>
                                {/foreach}
                            </tr>
                        {/foreach}
                    {/if}
