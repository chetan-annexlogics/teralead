                    ...
                    <div class="row entry clearfix history_widget" style="text-align: left"  data-history-id="{$HISTORY->getId()}">
                        ...
                        <div class='time_area'>
                            {if $TIME}
                                <p class="pull-right time_data">
                                    <span style="" title="{Vtiger_Util_Helper::formatDateTimeIntoDayString("$TIME")}">
                                          {CustomDashboards_Module_Model::formatDateDiffInStrings("$TIME")}
                                    </span>
                                </p>
                            {/if}
                        </div>
                         <div class='icon_area' style="margin-left: 5px;margin-right: 5px">
                        ...
                        <div class="mess_area">
                            ...
                            {else if $HISTORY->get('status') eq 6 || $HISTORY->get('status') eq 7}
                                {assign var=EMAIL value=CustomDashboards_History_Model::getRelatedRecordForHistory($HISTORY)}
                                {assign var=DATA_MODULE value=CustomDashboards_History_Model::getModuleNameForHistory($HISTORY->getParent()->get('parent_id'))}
                                <div>
                                    <b>{$USER->getName()}</b>
...
{*                {if $PARENT_ID != $OLD_PARENT_ID && $OLD_PARENT_ID != ''}<hr>{/if}*}
                <div class="row history_widget" style="text-align: left;"  data-history-id="{CustomDashboards_History_Model::getIdForComment($HISTORY)}">
