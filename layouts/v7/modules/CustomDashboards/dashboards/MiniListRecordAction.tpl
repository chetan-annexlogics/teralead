...
<td class="fake-body">
    ...
    <span class="more dropdown action">
         ...
         <ul class="dropdown-menu" data-widgetId="{$WIDGET->get('id')}"
             ...
             {if $RECORD->get('SMPButton')}
                 <hr>
                 {foreach from = $RECORD->get('SMPButton') item = BUTTON}
                     <style>
                         .p-o-smpbtn{$BUTTON['smpbuttonsid']}:hover {
                     ...
                     <li>
                         <a href="javascript:void(0);" data-module="{$RECORD->getModuleName()}"
                            data-button-id="{$BUTTON['smpbuttonsid']}" data-record-id="{$RECORD->get('id')}"
                            class="smpButtonQuickUpdateCustomDashboard p-o-smpbtn{$BUTTON['smpbuttonsid']}"
