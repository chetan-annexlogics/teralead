...
{strip}
	<div class="col-sm-12 col-xs-12 " style="margin-top:30px">
		...
		<div class="essentials-toggle hide" title="{vtranslate('LBL_LEFT_PANEL_SHOW_HIDE', 'Vtiger')}">
								...
								{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY from=$LISTVIEW_HEADERS}
									<th>
										...
										{elseif $DATA_TYPE == 'picklist'}
											{assign var=PICKLIST_VALUES value=CustomDashboards_Field_Model::getPicklistValueByField($LISTVIEW_HEADER_KEY)}
							...
							<tr class="emptyRecordsDiv">
								...
								<td colspan="{$COLSPAN_WIDTH}">
									<div class="emptyRecordsContent">
										...
										.{if $IS_MODULE_EDITABLE} <a
											href="{$MODULE_MODEL->getCreateRecordUrl()}"> {vtranslate('LBL_CREATE')} </a> {if Users_Privileges_Model::isPermitted($MODULE, 'Import') && $LIST_VIEW_MODEL->isImportEnabled()} {vtranslate('LBL_OR', $MODULE)}
										<a style="color:blue" href="#"
										   onclick="return Vtiger_Import_Js.triggerImportAction()"> {vtranslate('LBL_IMPORT', $MODULE)} </a>{vtranslate($MODULE, $MODULE)}{else}{vtranslate($SINGLE_MODULE, $MODULE)}{/if}{/if}
									</div>
								</td>
							</tr>
						{/if}
