Vtiger.Class("CustomDashboards_ButtonDashBoard_Js", {
    ...
    registerEventsQuickUpdate:function(){
        ...
        jQuery(document).off('click','.smpButtonQuickUpdateCustomDashboard');
        jQuery(document).on('click','.smpButtonQuickUpdateCustomDashboard',function(e){
            var target = jQuery(e.currentTarget);
            var postSMPButtonsSave  = function(data) {
                ...
                function(err,data) {
                    if(err == null){
                        if(data.automated_update_field == '' && data.field_name == ''){
                            ...
                            params['smpbuttons_id'] = smpbuttonid;
                            ...
                            var smpButtonId = smpbuttonid;
                            var viewEditUrl = "module=SMPButtons&view=QuickEditAjax&record="+source_recordId+"&moduleEditName="+source_module+"&smpButtonId="+smpButtonId;
                            var params= {'callbackFunction': postSMPButtonsSave,'noCache':true};
                            thisInstance.getSMPButtonsQuickEditForm(viewEditUrl, source_module, params).then(function(data) {
                                thisInstance.handleSMPButtonsQuickEditData(data, params);
                                var form = jQuery("#smpButtonQuickEdit");
    ...
    handleSMPButtonsQuickEditData: function(data, params) {
        if (typeof params == 'undefined') {
            params = {};
        }
        var thisInstance = this;
        app.helper.showModal(data,{'cb' : function (data){
                var quickEditForm = data.find('form[name="smpButtonQuickEdit"]');
                app.event.trigger('post.smpButtonQuickEditForm.show',quickEditForm);
                ...
                thisInstance.registerSMPButtonsPostLoadEvents(quickEditForm, params);
    ...
    registerSMPButtonsPostLoadEvents: function(form, params) {
        var thisInstance = this;
        ...
        form.find("button[name='smpButtonsSave']").on('click', function(e) {
            var form = jQuery(e.currentTarget).closest('form');
            var module = form.find('[name="module"]').val();
            var aDeferred = jQuery.Deferred();
            var params = {
                submitHandler: function (frm) {
                    jQuery("button[name='smpButtonsSave']").attr("disabled", "disabled");
    ...
    getSMPButtonsQuickEditForm: function(url, moduleName, params) {
        var thisInstance = this;
        var aDeferred = jQuery.Deferred();
        var requestParams;
        if (typeof params == 'undefined') {
...
jQuery(document).ready(function () {
    // Only load when have SMPButtons
    var instance = new CustomDashboards_ButtonDashBoard_Js();
