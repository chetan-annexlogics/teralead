 ...
 * All Rights Reserved.
 *************************************************************************************/
Vtiger_List_Js("CustomDashboards_List_Js",{

    listInstance : false,

    addReport : function(url){
        var listInstance = CustomDashboards_List_Js.getInstance();
        ...
        if(validationResult != true){
            ...
            app.request.post(params).then(function(error,data) {
                ...
                var callBackFunction = function(data){
                    var reportsListInstance = new CustomDashboards_List_Js();
    ...
    registerEditFilterClickEvent : function(){
        ...
        listViewFilterBlock.on('mouseup','li i.editFilter',function(event){
            ...
            CustomDashboards_List_Js.triggerAddFolder(editUrl);
    ...
    savePinToDashboard : function(parentElement,element, customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
            if (data.duplicate) {
                var params = {};
                if(type == 'pivot'){
                    params = {message: app.vtranslate('JS_PIVOT_ALREADY_PINNED_TO_DASHBOARD', 'CustomDashboards')+' In Tab ' + tabName};
                }
                else{
                    params = {message: app.vtranslate('JS_CHART_ALREADY_PINNED_TO_DASHBOARD', 'CustomDashboards')+' In Tab ' + tabName};
                }
                ...
                var title = '';
                if(type == 'pivot'){
                    message = app.vtranslate('JS_PIVOT_PINNED_TO_DASHBOARD', 'CustomDashboards');
                    title = app.vtranslate('JSLBL_UNPIN_PIVOT_FROM_DASHBOARD')
                }
                else{
                    message = app.vtranslate('JS_CHART_PINNED_TO_DASHBOARD', 'CustomDashboards');
    ...
    unpinFromDashboard : function (parentElement,element,customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
            if(data.unpinned) {
                ...
                var title = '';
                if(type == 'pivot'){
                    message = app.vtranslate('JS_PIVOT_REMOVED_FROM_DASHBOARD', 'CustomDashboards') + " In Tab " + tabName;
                    title = app.vtranslate('JSLBL_PIN_PIVOT_TO_DASHBOARD','CustomDashboards');
                }else{
                    message = app.vtranslate('JS_CHART_REMOVED_FROM_DASHBOARD', 'CustomDashboards')+ " In Tab " + tabName;
                    title = app.vtranslate('JSLBL_PIN_CHART_TO_DASHBOARD','CustomDashboards');
                }
                app.helper.showSuccessNotification({message:message});
