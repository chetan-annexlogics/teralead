    ...
    return (function e(t, n, r) {
        ...
        2: [function(_dereq_, module, exports) {
            ...
            var Aggregations = module.exports = {
                ...
                sum: function(datafield, intersection, datasource) {
                    ...
                    var regexDigitGroupingSeparator = new RegExp('\\'+digitGroupingSeparator,'gm');
                    var regexDecimalSeparator = new RegExp('\\'+decimalSeparator,'gm');
                    forEachIntersection(datafield, intersection, datasource, function(val) {
                        if(val) {
                            val = val.toString();
                            val = val.replace(regexDigitGroupingSeparator,'');
                            val = val.replace(regexDecimalSeparator,'.');
                            sum += parseFloat(val);
                        }
                    });
                    if(typeof sum == 'number'){
                        ...
                        if(truncateDecimals == 1){
                            sum = sum.toFixed(0);
                            sum = sum.toString();
                            sum = sum.replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1"+digitGroupingSeparator);
                        }else{
                            sum = sum.toFixed(numberDecimals);
                            sum = sum.toString();
                            sum = sum.replace(/(\d)(?=(\d{3})+(?!\d)(?=\W))/g, "$1"+digitGroupingSeparator);
                        }
                        sum = sum.replace('.', decimalSeparator);
                    }
                    return sum;
                },
        ...
        6: [function(_dereq_, module, exports) {
            ...
            module.exports = function(pgridwidget) {
                ...
                var headerStyle = 'style="border: 0.5px solid black;background-color: ' + themeFadeout + ';" bgcolor="' + themeFadeout + '"';
                ...
                var rowHeadersAndDataCells = (function() {
                    ...
                    for (var i = 0; i < pgridwidget.rows.headers.length; i++) {
                        ...
                        rowStr += dataRow.reduce(function(tr, dataCell, index) {
                            ...
                            return (tr += '<td style="border: 0.5px solid black;">' + value + '</td>');
                        }, '');
                        str += rowStr + '</tr>';
                    }
                    return str;
