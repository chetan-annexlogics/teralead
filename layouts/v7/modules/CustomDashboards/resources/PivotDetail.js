...
CustomDashboards_Detail_Js("CustomDashboards_PivotDetail_Js", {
    selectRow : '',
    selectDataField : '',
    /**
    ...
    init : function() {
        this._super();
        CustomDashboards_PivotDetail_Js.currentInstance = this;
    },
    ...
    registerPushValueToSortBy: function (element){
        ...
        if (identified == 'groupbyfield_rows') {
            var selectedData = CustomDashboards_PivotDetail_Js.currentInstance.selectDataRow;
        } else if (identified == 'datafields'){
            var selectedData = CustomDashboards_PivotDetail_Js.currentInstance.selectDataField;
        }
        var data = select.val();
        if (data == null) {
            difference = data;
        ...
        if (identified == 'groupbyfield_rows') {
            CustomDashboards_PivotDetail_Js.currentInstance.selectDataRow = data;
        } else if (identified == 'datafields'){
            CustomDashboards_PivotDetail_Js.currentInstance.selectDataField = data;
        }
    ...
    registerUpdateSelectElementEventForRows: function() {
        ...
        jQuery("#groupbyfield_rows , #sort_by").on("change", function(e) {
            ...
            CustomDashboards_PivotEdit3_Js.updateSelectElement(valueOption, fieldsColumn);
    ...
    registerUpdateSelectElementEventForColumns: function() {
        ...
        jQuery("#groupbyfield_columns").on("change", function(e) {
            ...
            CustomDashboards_PivotEdit3_Js.updateSelectElement(valueOption, fieldsRow);
        });
    },
    ...
    savePinToDashBoard : function(element,customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
            if (data.duplicate) {
                var params = {
                    message: app.vtranslate('JS_PIVOT_ALREADY_PINNED_TO_DASHBOARD', 'CustomDashboards')
                };
                app.helper.showSuccessNotification(params);
            } else {
                var message = app.vtranslate('JS_PIVOT_PINNED_TO_DASHBOARD', 'CustomDashboards') +" In Tab " + tabName;
    ...
    unpinFromDashboard : function (element,customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
            if(data.unpinned) {
                var message = app.vtranslate('JS_PIVOT_REMOVED_FROM_DASHBOARD', 'CustomDashboards') +" In Tab " + tabName;
                app.helper.showSuccessNotification({message:message});
    ...
    registerEventsDeleteChart: function () {
        ...
        if($('[name="datachart"]').val()){
            ...
            $('.deleteChart').click(function (e) {
                ...
                $('.deleteChart').remove();
                var message = app.vtranslate('JSLBL_CHART_DELETE_SUCCESS', 'CustomDashboards');
                app.helper.showSuccessNotification({message:message});
            })
        }
    },
    registerEvents: function () {
        ...
        CustomDashboards_PivotEdit3_Js.registerFieldForChosen();
        CustomDashboards_PivotEdit3_Js.initSelectValues();
