Vtiger.Class("CustomDashboards_DashBoard_Js", {
    gridstack : false,
    ...
    addWidget : function(element, url) {
        ...
        var activeTabId = CustomDashboards_DashBoard_Js.currentInstance.getActiveTabId();
        ...
        if (miniListDentified == 'KeyMetrics') {
            ...
            app.request.post({"url":"index.php?module=CustomDashboards&view=KeyMetricsWizard&widgetName=KeyMetrics"}).then(function (err, res) {
                ...
                thisInstance.registerWidgetColorPickerEvent(widgetContainer);
                ...
                $('[name="saveSettingWidget"]').on("click",function () {
                    ...
                    var widgetcolor = $('input[name="widgetcolor"]').val();
                    if (widgetcolor) url += '&widgetcolor='+widgetcolor;
                    ...
                    app.request.post({"url": url}).then(function (err, res) {
                        ...
                        CustomDashboards_DashBoard_Js.gridstack.addWidget(widgetContainer,0,0, 2, 2);
                        widgetContainer.find('div.panel').html(res);
                        CustomDashboards_DashBoard_Js.currentInstance.loadWidget(widgetContainer);
                        CustomDashboards_DashBoard_Js.gridstack.movable(widgetContainer,true);
                        CustomDashboards_DashBoard_Js.gridstack.resizable(widgetContainer,true);
                        CustomDashboards_DashBoard_Js.currentInstance.showButtonSavePositionWidgets(widgetContainer);
            ...
            app.request.post({"url": url}).then(function (err, res) {
                ...
                element.show();
                widgetContainer.data('url',url);
                CustomDashboards_DashBoard_Js.gridstack.addWidget(widgetContainer,0,0, width, height);
            ...
            app.helper.showProgress();
            app.request.post({"url":"index.php?module=CustomDashboards&view=GaugeWizard&widgetName=Gauge&step=step1" }).then(function (err, res) {
                app.helper.hideProgress();
                app.helper.showModal(res,{"cb": function (data) {
                    ...
                    var targetReportSelect2 = vtUtils.showSelect2ElementView(targetReportSelectDOM, {
                        placeholder: app.vtranslate('JS_SELECT_REPORT','CustomDashboards')
                    });
                ...
                $('[name="saveSettingWidget"]').on("click",function () {
                    ...
                    if (selectedColor) url += '&color='+selectedColor;
                    var widgetcolor = $('input[name="widgetcolor"]').val();
                    if (widgetcolor) url += '&color='+widgetcolor;
                    var refresh_time = $('select[name="refresh_time"]').val();
                    if (refresh_time) url += '&time='+refresh_time;
                    url += '&modeWidget=add&tabid='+activeTabId;
                    app.request.post({"url": url}).then(function (err, res) {
    ...
    addMiniListWidget: function(element, url) {
        ...
        var instance = this;
        app.request.post({"url": "index.php?module=CustomDashboards&view=MiniListWizard&step=step1"}).then(function (err, res) {
            if (err == null) {
                setTimeout(function () {
                    app.helper.showModal(res, {"cb": function (data) {
                            var wizardContainer = jQuery(data);
    ...
    saveSelectedColor: function (){
        $('div.color-box').on('click',function () {
            $('div.color-box').removeClass('fa-check-square');
            var instance = $(this);
			instance.addClass('fa');
            instance.addClass('fa-check-square');
            var color = instance.data('value');
            $('input[name="selectedColor"]').val(color);
        });
    },

    saveSelectedWidgetColor: function (){
        $('div.color-box-widget').on('click',function () {
            $('div.color-box-widget').removeClass('fa-check-square');
            var instance = $(this);
            instance.addClass('fa');
            instance.addClass('fa-check-square');
            var color = instance.data('value');
            $('input[name="widgetcolor"]').val(color);
        });
    },
    ...
    registerSaveSettingWidget: function(widgetId,urlParams,parentGrid){
        ...
        $('button[name="saveSettingWidget"]').on('click',function () {
            ...
            if(selectedModule){
                ...
                data = {targetReport:targetReport,dataGauge:dataGauge,dataColor:dataColor,sub_title:sub_title,backgroundColor:backgroundColor,decimal:decimal,formatLargeNumber:formatLargeNumber,icon:icon,filterAssignedto:filterAssignedto,filterCreatedby:filterCreatedby};
            }else {
            ...
            app.request.post({data:params}).then(function () {
                app.request.post({"url":urlParams}).then(function(err,data){
                    ...
                    if(minHeight != '' && maxHeight != ''){
                        if(jQuery(parentGrid).data('widget-type') == 'Mini List CustomDashboards' || widgetName == 'MiniList'){
                    ...
                    var obj = new CustomDashboards_DashBoard_Js();
    ...
    registerColorPickerEvent : function(container) { 
        ...
        container.find('#div-picklist-color .colorPicker ').removeClass('hide');
        container.find('#div-picklist-color').removeClass('hide');
    ...
    registerWidgetColorPickerEvent : function(container) {
        var colorPickerDiv = container.find('.widgetcolorPicker');
        container.find('#div-picklist-color .colorPicker ').addClass('hide');
        container.find('#div-picklist-color').addClass('hide');
        var selectedColorElement = container.find('[name=widgetcolor]');
        app.helper.initializeColorPicker(colorPickerDiv, {}, function(hsb, hex, rgb) {
            var selectedColorCode = '#'+hex;console.info(hex);
            selectedColorElement.val(selectedColorCode);
        });
        var color = selectedColorElement.val();
        if(!color) {
    ...
    registerEventShowWidgetCustomColor : function(){
        var thisInstance = this;
        var container = $('.form-horizontal');
        container.find('label#widget_color').hide();
        container.find('div#widget_color').hide();
        container.find('#label-picklist-color').show();
        container.find('#div-widget-picklist-color').show();
        thisInstance.registerWidgetColorPickerEvent(container);
        container.find('#collorpicker_272').hide();
    },
    ...
    registerEventShowWidgetColor : function(){
        var thisInstance = this;
        var container = $('.form-horizontal');
        container.find('label#widget_color').show();
        container.find('div#widget_color').show();
        container.find('#label-picklist-color').hide();
        container.find('#div-widget-picklist-color').hide();
        thisInstance.saveSelectedWidgetColor(container);
        container.find('#collorpicker_802').hide();
    },
    ...
    eventActionHeaderWidget : function (element) {
        ...
        if (linkId != undefined){
            ...
            var url = 'index.php?module=CustomDashboards&action=RemoveWidget&linkid='+linkId+'&widgetid='+data_record+'';
        }
        else {
        ...
        if(event == 'detail'){
            ...
            app.request.post({data:data}).then( function(err,data){
                ...
                app.helper.showModal(data,{cb : function() {
                        ...
                        var selected_color = $('input[name="widgetcolor"]').val();
                        $('div.color-box-widget').each(function () {
                            var color = $(this).data('value');
                            if (color == selected_color) {
                                $(this).addClass('fa');
                                $(this).addClass('fa-check-square');
                        ...
                        thisInstance.registerEventShowWidgetColor(container);
            ...
            app.request.post({"url":urlParams}).then(function(err,data){
                ...
                CustomDashboards_DashBoard_Js.currentInstance.registerLoadMore();
                CustomDashboards_DashBoard_Js.currentInstance.registerPicklistValueDropMenu();
            });
        }
        else{
            var message = app.vtranslate('Are you sure remove this widget ?', 'CustomDashboards');
                ...
                function(e) {
                    ...
                    app.request.post({url:url}).then(function(err, res) {
                        if(!err){
                            ...
                            var message = app.vtranslate('CUSTOMDASHBOARD_WIDGET_REMOVE_SUCCESS', 'CustomDashboards');
                            app.helper.hideProgress();
    ...
    registerMoreClickEvent: function (e) {
        ...
        var params = {
            ...
            'tabid': CustomDashboards_DashBoard_Js.currentInstance.getActiveTabId()
        };
    ...
    init : function() {
        ...
        CustomDashboards_DashBoard_Js.currentInstance = this;
    },
    ...
    resizeGrid : function () {
        ...
        var grid = CustomDashboards_DashBoard_Js.gridstack;
        if (isBreakpoint('xs')) {
    ...
    registerLazyLoadWidgets : function() {
        var thisInstance = this;
        jQuery('.dashBoardTabContents').bind("scroll", function() {
    ...
    registerFilter : function(container) {
        ...
        var pickerParams = {
            format : CustomDashboards_DashBoard_Js.getUserDateFormat(),
        };
        ...
        dateRangeElement.attr('data-date-format',CustomDashboards_DashBoard_Js.getUserDateFormat());
    },
    ...
    loadWidget : function(widgetContainer) {
        ...
        app.request.post({"url":urlParams}).then(
            function(err,data){
                ...
                if (time > 5000) {
                    CustomDashboards_DashBoard_Js.intervalVal[miniListId] = setInterval(function () {
                ...
                thisInstance.registerPicklistValueDropMenu();
                if (widgetModeName != 'Gauge'){
                    CustomDashboards_DashBoard_Js.makePerfectScrollbar(contentContainer);
                }
                var minHeight = widgetContainer.data('min-height');
                var maxHeight = widgetContainer.data('max-height');
                var indexnodes = 0;
                var nodes = CustomDashboards_DashBoard_Js.gridstack.grid.nodes;
                $(nodes).each(function (index,value) {
                    if($(this.el[0]).data('record') == miniListId){
                        indexnodes = index;
                ...
                if(jQuery(widgetContainer).data('widget-type') == 'Mini List CustomDashboards' || widgetModeName == 'MiniList'){
                    var currentWidgetHeight = jQuery(widgetContainer).height();
                    var tableHeight = jQuery(widgetContainer).find('table').height();
                    if(tableHeight + 29 + 10 > currentWidgetHeight){
                        var height = Math.ceil((((tableHeight + 29 + 10) - 60) / 80) + 1);
                        if(height > minHeight && height <= maxHeight){
                            CustomDashboards_DashBoard_Js.gridstack.resize(CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].el,CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].width,height)
                        }else if(height >= maxHeight){
                            CustomDashboards_DashBoard_Js.gridstack.resize(CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].el,CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].width,maxHeight)
                        }else if (height < minHeight){
                            CustomDashboards_DashBoard_Js.gridstack.resize(CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].el,CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].width,minHeight)
                        }
                    }else{
                        CustomDashboards_DashBoard_Js.gridstack.resize(CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].el,CustomDashboards_DashBoard_Js.gridstack.grid.nodes[indexnodes].width,minHeight)
                    }
    ...
    registerAddDashboardTab : function(){
        ...
        dashBoardContainer.on("click",".addNewDashBoard",function(e){
            ...
            if(jQuery('.dashboardTab').length >= CustomDashboards_DashBoard_Js.dashboardTabsLimit ){
                    ...
                    var cb = function(data){
                        ...
                        var params = {
                            submitHandler : function(form){
                                ...
                                if (action == 'duplicate') {
                                    paramsForDup['duplicateTabId'] = CustomDashboards_DashBoard_Js.currentInstance.getActiveTabId();
                                }
            ...
            app.request.post({"data":data}).then(function(err,res){
                if(err === null){
                    var cb = function(data){
                        var form = jQuery(data).find('#EditBoard');
                        var params = {
                            submitHandler : function(form){ console.info(form);
                                var labelEle = jQuery(form).find('[name="boardName"]');
                                var boardName = labelEle.val().trim();console.info(boardName);
                                if(boardName.length > 30) {
                                    vtUtils.showValidationMessage(labelEle, app.vtranslate('JS_TAB_LABEL_EXCEEDS_CHARS', 50), {
                                        position: {
                                            my: 'bottom left',
                    ...
                    form.find('[name="select-board"]').on('change',function () {
                        ...
                        app.request.post({"data":params}).then(function(err,res){
                            if(res){
                                ...
                                form.find('[name="modules[]"]').val(res.modules.split(','));
                                form.find('[name="modules[]"]').select2();
    ...
    registerDashBoardTabClick : function(){
        ...
        dashBoardContainer.off("shown.bs.tab").on("shown.bs.tab",".dashboardTab",function(e){
            for(var key in CustomDashboards_DashBoard_Js.intervalVal) {
                clearInterval(CustomDashboards_DashBoard_Js.intervalVal[key]);
            }
            var currentTarget = jQuery(e.currentTarget);
            var tabid = currentTarget.data('tabid');
            var boardid = $('select[name="header-board"]').val();
            app.changeURL("index.php?module=CustomDashboards&view=DashBoard&boardid="+boardid+"&tabid="+tabid);
            ...
            if(typeof jQuery("#tab_"+tabid).find(".dashBoardTabContainer").val() !== 'undefined'){
                ...
                CustomDashboards_DashBoard_Js.setGridstack(tabid);
            ...
            app.request.post({"data":data}).then(function(err,data){
                if(err === null){
                    ...
                    $('head').find('title').text(thisInstance.getActiveTabName());
					thisInstance.registerLazyLoadWidgets();
                }
            });
        });
    },
    ...
    registerChangeBoard:function(){
        ...
        $('.selectboard').on('click',function () {
			...
			$('.selectboard').removeClass('selected');
            $(this).addClass('selected');
            $('[name="header-board"]').val($(this).data('id')).trigger('change');
        ...
        if(boardid != $('[name="header-board"]').val() && thisInstance.getUrlParameter('boardid') != undefined){
            app.changeURL("index.php?module=CustomDashboards&view=DashBoard&boardid="+1+"&tabid="+tabid);
                            ...
                            submitHandler : function(form){
                                ...
                                app.request.post({"data":params}).then(function (err,data) {
                                    ...
                                    if(err) {
                                        ...
                                        // dashBoardContainer.find('#tab_'+activeTabId).find('div[data-widget-type="Mini List CustomDashboards"]  .action-widget-header[data-event="Refresh"]').trigger('click');
                                    }
                                });
                            }
                        };
    ...
    showDidePreViewHistory : function(contentContainer){
        if(!CustomDashboards_DashBoard_Js.gridstack)
            return false;
    ...
    showHideHeaderActionsWidgets : function() {
        if(!CustomDashboards_DashBoard_Js.gridstack)
            return false;
        var container = CustomDashboards_DashBoard_Js.gridstack.container;
    ...
    registerEvents : function () {
        ...
        app.event.on("post.DashBoardTab.load",function(event, dashBoardInstance){
            ...
            var tabIdActive = instance.getActiveTabId();console.info(tabIdActive);
            instance.registerNotificationDynamic(tabIdActive)
            CustomDashboards_DashBoard_Js.setGridstack(tabIdActive);
            instance.init();
