...
CustomDashboards_Detail_Js("CustomDashboards_ChartDetail_Js", {
    gridstack : false,
    selectGroupByField: '',
    selectDataField : '',
    ...
    printAndExportChat: function (e) {
        ...
        if (chartCanvas) {
            ...
            var message = app.vtranslate('JS_DATA_NOT_AVAILABLE', 'CustomDashboards');
    ...
    init : function() {
        this._super();
        CustomDashboards_ChartDetail_Js.currentInstance = this;
    },
	/**
	...
	getInstance: function () {
		...
		var chartClass = window["CustomDashboard_" + chartClassName + "_Js"];
    ...
    registerPushValueToSortBy: function (element){
        ...
        if (identified == 'groupbyfield') {
            if (typeof CustomDashboards_ChartDetail_Js.currentInstance.selectGroupByField == "string"){
                CustomDashboards_ChartDetail_Js.currentInstance.selectGroupByField= JSON.parse('[" '+ CustomDashboards_ChartDetail_Js.currentInstance.selectGroupByField +' "]')
            }
            var selectedData = CustomDashboards_ChartDetail_Js.currentInstance.selectGroupByField;
        } else if (identified == 'datafields'){
            var selectedData = CustomDashboards_ChartDetail_Js.currentInstance.selectDataField;
        }
        var data = select.val();
        ...
        if (identified == 'groupbyfield') {
            CustomDashboards_ChartDetail_Js.currentInstance.selectGroupByField = data;
        } else if (identified == 'datafields'){
            CustomDashboards_ChartDetail_Js.currentInstance.selectDataField = data;
        }
		...
		jQuery('.generateReportChart').on('click', function (e) {
			...
			app.request.post({data: postData}).then(
					function (error, data) {
                        ...
                        CustomDashboards_ChartDetail_Js.setGridstack();
	...
	registerEventForChartGeneration: function () {
		...
		if(!dataChart){
			CustomDashboards_ChartDetail_Js.displayNoDataMessage();
			return false;
        }
        return true;
	},
	...
	savePinToDashBoard : function(element,customParams) {
		...
		app.request.post({data: params}).then(function (error,data) {
			if (data.duplicate) {
				var params = {
						message: app.vtranslate('JS_CHART_ALREADY_PINNED_TO_DASHBOARD', 'CustomDashboards')
				};
				app.helper.showSuccessNotification(params);
			} else {
				var message = app.vtranslate('JS_CHART_PINNED_TO_DASHBOARD', 'CustomDashboards') +" In Tab " + tabName;
	...
	unpinFromDashboard : function (element,customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
			if(data.unpinned) {
				var message = app.vtranslate('JS_CHART_REMOVED_FROM_DASHBOARD', 'CustomDashboards') +" In Tab " + tabName;
				app.helper.showSuccessNotification({message:message});
    ...
    registerEventAddGroup : function () {
        ...
        jQuery('button[name=addgroup]').on('click',function () {
            ...
            CustomDashboards_AdvanceFilter_Js.getInstance(jQuery('.groupCondition'+filterContainerLength,container),true);
    ...
    resizeGrid : function () {
        ...
        var grid = CustomDashboards_ChartDetail_Js.gridstack;
        if (isBreakpoint('xs')) {
	...
	registerEvents: function () {
		...
		CustomDashboards_ChartEdit3_Js.registerFieldForChosen();
		CustomDashboards_ChartEdit3_Js.initSelectValues();
		var chartEditInstance = new CustomDashboards_ChartEdit3_Js();
