 ...
 * All Rights Reserved.
 *************************************************************************************/
CustomDashboards_Edit_Js("CustomDashboards_Edit2_Js",{},{
	...
	calculateValues : function(){
		...
		jQuery.each(calculationFieldRows,function(index,element){
			var calculationTypes = jQuery(element).find('.calculationType:checked:not([name*="rename_columns_selected["])');
	...
	registerEventRenameColumns : function () {
		var container = this.getContainer();
		var tableCalculations = container.find('table.CalculationFields');
		var checkboxRenameColumns = tableCalculations.find('input[name*="rename_columns_selected"]');
        checkboxRenameColumns.on('change',function (e) {
			var element = $(this);
            var trElement = element.closest('tr');
			if(element.prop('checked') == true){
				trElement.find('span.default-columns-name').hide();
				trElement.find('input[name*="rename_columns["]').show();
			}else{
                trElement.find('span.default-columns-name').show();
                trElement.find('input[name*="rename_columns["]').hide();
			}
        })
    },
	registerEvents : function(){
        ...
        this.registerEventRenameColumns();
	}
});
