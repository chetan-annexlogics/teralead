...
Vtiger_Detail_Js("CustomDashboards_SqlReportDetail_Js",{},{
    unpinFromDashboard : function (element,customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
            if(data.unpinned) {
                var message = app.vtranslate('JS_SQL_REPORT_REMOVED_FROM_DASHBOARD', 'CustomDashboards') +" In Tab " + tabName;
    ...
    savePinToDashBoard : function(element,customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
            if (data.duplicate) {
                var params = {
                    message: app.vtranslate('JS_SQL_REPORT_ALREADY_PINNED_TO_DASHBOARD', 'CustomDashboards')
                };
                app.helper.showSuccessNotification(params);
            } else {
                var message = app.vtranslate('JS_SQL_REPORT_PINNED_TO_DASHBOARD', 'CustomDashboards') +" In Tab " + tabName;
                app.helper.showSuccessNotification({message:message});
