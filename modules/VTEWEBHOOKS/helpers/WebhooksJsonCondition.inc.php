...
class WebhooksJsonCondition {

    function __construct() {

    }

    function evaluate($condition, $entityData, $oldData) {
            ...
            foreach ($expr as $cond) {
                ...
                if (count($matches) == 0) {
                    if ($cond['fieldname'] == '_VT_add_comment' && $cond['isEvaluated']) {
                        ...
                        $expressionResults[$conditionGroup][$i]['result'] = $this->checkCondition($entityData, $oldData, $cond);
                    }
                } else {
                    ...
                    if ($referenceFieldId != 0) {
                        if(is_array($referenceFieldId)) {
                            ...
                            foreach ($referenceFieldId as $key=>$value) {
                                ...
                                if ($entityData->getModuleName() == $referenceModule) {
                                    $cond['fieldname'] = $fieldname;
                                    $returnValue = $this->checkCondition($entity, $oldData, $cond, $entityData);
                                    if($returnValue) {
                            ...
                            if ($entityData->getModuleName() == $referenceModule) {
                                $cond['fieldname'] = $fieldname;
                                $expressionResults[$conditionGroup][$i]['result'] = $this->checkCondition($entity, $oldData, $cond, $entityData);
                            } else {
    ...
    function checkCondition($entityData,$oldData , $cond, $referredEntityData=null) {
        $data = $entityData->getData();
        ...
        switch ($condition) {
            ...
            case "is":
                if (preg_match('/([^:]+):boolean$/', $value, $match)) {
					...
					if($fieldInstance && $fieldInstance->getFieldDataType() == 'boolean') {
                        $fieldValue = ($fieldValue == 'on') ? '1' : '0';
                    }
                    return $fieldValue == $value;
                }
            case "is not":
            ...
            case "has changed" :
                ...
                $oldEntityData = VTEntityData::fromEntityId($adb, $recordId, $moduleName);

                foreach ($oldData as $fieldname => $fieldvalue){
                    $oldEntityData->set($fieldname,$fieldvalue);
                }

                if($entityData->getModuleName() == 'HelpDesk') {
                    $oldEntityData->set('comments', getTicketComments($recordId));
                } elseif($entityData->getModuleName() == 'Invoice'){
                    $oldEntityData->set('invoicestatus', getInvoiceStatus($recordId));
                }
                $hasChanged = $oldEntityData->get($cond['fieldname']) != $entityData->get($cond['fieldname']);
