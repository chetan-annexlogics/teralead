...
class PBXManager_ListView_Model extends Vtiger_ListView_Model {
   ...
   public function getListViewEntries($pagingModel) {
        ...
        foreach ($listViewEntries as $recordId => $record) {
            ...
            if ($url[0] != '' && ($listViewEntries[$recordId]['callstatus'] == 'ANSWERED' || $listViewEntries[$recordId]['callstatus'] == 'VOICE RECORD')) {
                $detailView = new PBXManager_Detail_View();
                $url = $detailView->downloadCallRecord($recordId);
                $html = "<div><audio controls style='width:250px;height:30px;'><source src='".$url."' type='audio/mpeg'></audio></div>";
                $listViewEntries[$recordId]['recordingurl'] = $html;
            } else {
