...
class CustomDashboards_Chart_Model extends Vtiger_Base_Model
{
    public $sort = NULL;
    public $limit = NULL;
    public $order = NULL;
    ...
    public static function getInstanceById($reportModel)
    {
        ...
        $result = $db->pquery("SELECT * FROM vtiger_customdashboardtype WHERE reportid = ?", array($reportModel->getId()));
    ...
    public function generateChartForHTML($data)
    {
        ...
        $htmlChart = "<script src=\"" . $site_URL . "/layouts/v7/modules/CustomDashboards/resources/chartjs/Chart.bundle.min.js\"></script>\n                        <script src=\"" . $site_URL . "/layouts/v7/modules/CustomDashboards/resources/chartjs/Chart.BarFunnel.min.js\"></script>\n                        <script src=\"" . $site_URL . "/layouts/v7/modules/CustomDashboards/resources/chartjs/utils.js\"></script>";
        if ($data["chart_group_type"] == "pie") {
            $htmlChart .= "<script src=\"" . $site_URL . "/layouts/v7/modules/CustomDashboards/resources/chartjs/Chart.Funnel.bundle.min.js\"></script>";
            $htmlChart .= "<script src=\"" . $site_URL . "/layouts/v7/modules/CustomDashboards/resources/chartjs/chartjs-piecelabel.js\"></script>";
        }
        $htmlChart .= "<script>";
        if ($data["chart_group_type"] == "pie") {
...
abstract class Base_Chart extends Vtiger_Base_Model
{
    ...
    public function setReportRunObject()
    {
        ...
        $this->reportRun = CustomDashboardRun::getInstance($reportModel->get("reportid"));
    }
    ...
    public function getQuery()
    {
        $chartModel = $this->getParent();
        $reportModel = $chartModel->getParent();
        $this->reportRun = CustomDashboardRun::getInstance($reportModel->getId());
        ...
        if ($chartModel->sort) {
            foreach ($chartModel->sort as $index => $item) {
                ...
                if (1 < sizeof($checkItem)) {
                    ...
                    $tempVal = split(" AS ", $sortFieldSql);
                    $chartModel->sort[$index] = $tempVal[1];
                }
            }
...
class BarChart extends Base_Chart
{
    ...
    {
        global $adb;
        $rename_field_result = $adb->pquery("SELECT rename_field_chart  FROM vtiger_customdashboardtype WHERE reportid = ?", array($recordId));
    ...
    public function getDataLabels()
    {
        ...
        if ($this->isRecordCount()) {
            $dataLabels[] = vtranslate("LBL_RECORD_COUNT", "CustomDashboards");
        }
        $queryColumnsByFieldModel = $this->getQueryColumnsByFieldModel();
                    ...
                    foreach ($renameFieldCharts as $key => $renameFieldChart) {
                        if ($reportColumn == $renameFieldChart->renameSelectChart) {
                            if ($renameFieldChart->translatedLabel != "") {
                                ...
                                $dataLabels[] = vtranslate($aggregateFunctionLabel, "CustomDashboards", $fieldTranslatedLabel);
                            }
                        }
                    }
                } else {
