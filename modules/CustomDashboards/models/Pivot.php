 ...
 * Time: 11:19 AM
 */
class CustomDashboards_Pivot_Model extends Vtiger_Base_Model
{
    public $sort = NULL;
    public $limit = NULL;
    public $order = NULL;
    ...
    public function getData()
    {
        ...
        return $pivotModel->generateData();
    }
    public function getRenameField($recordId)
    {
        global $adb;
        $rename_field_result = $adb->pquery("SELECT rename_field  FROM vtiger_customdashboardtype WHERE reportid = ?", array($recordId));
...
class Base_Pivot extends Vtiger_Base_Model
{
        ...
        if (is_array($groupByColumnsByFieldModel)) {
            foreach ($groupByColumnsByFieldModel as $groupField) {
                ...
                if (($groupField->getFieldDataType() == "picklist" || $groupField->getFieldDataType() == "multipicklist") && vtws_isRoleBasedPicklist($groupField->getName())) {
                    ...
                    $dataGeneratePivot["nonMultiPicklist"][] = $valuePicklistField;
                }
            ...
            if ($groupColumnsField) {
                foreach ($groupColumnsField as $key => $columsFieldModel) {
                    ...
                    if ($fieldDataType == "picklist") {
                        ...
                        $labelColumn = vtranslate($row[strtolower($groupColumns[$key][0])], $columsFieldModel->getModuleName());
                    } else {
                        if ($fieldDataType == "multipicklist") {
                            $multiPicklistValue = $row[strtolower($groupColumns[$key][0])];
                            ...
                            if ($fieldDataType == "multireference") {
                                ...
                                if ($fieldDataType == "date") {
                                    ...
                                    if ($fieldDataType == "datetime") {
                                        ...
                                        $labelColumn = decode_html(str_replace("\\", "\\\\", $row[strtolower($groupColumns[$key][0])]));
                                    }
                                }
                            }
                        }
            ...
            if ($groupRowsField) {
                foreach ($groupRowsField as $key => $rowsFieldModel) {
                    ...
                    if ($fieldDataType == "picklist") {
                        ...
                        $labelRow = vtranslate($row[strtolower($groupRows[$key][0])], $rowsFieldModel->getModuleName());
                    } else {
                        if ($fieldDataType == "multipicklist") {
                            $multiPicklistValue = $row[strtolower($groupRows[$key][0])];
                            $multiPicklistValues = explode(" |##| ", $multiPicklistValue);
                            foreach ($multiPicklistValues as $multiPicklistValue) {
                            ...
                            if ($fieldDataType == "multireference") {
                                ...
                                if ($fieldDataType == "date") {
                                    ...
                                    if ($fieldDataType == "datetime") {
                                        ...
                                        $labelRow = decode_html(str_replace("\\", "\\\\", $row[strtolower($groupRows[$key][0])]));
                                    }
                                }
                            }
                        }
    ...
    public function generatePivot($data)
    {
        ...
        $htmlExportPivotBody = "<a class=\"btn btn-default\" download=\"pivot_" . $recordId . "_" . $currentDate . ".xls\" href=\"#\" onclick=\"return exportToExcel(this);\">Export EXCEL</a>";
        $htmlExportPivotBottom = "<script type=\"text/javascript\">";
        $htmlExportPivotBottom .= "function exportToExcel(anchor) {\n                                        anchor.href = orb.export(pgridwidget);\n                                        return true;\n                                   }";
