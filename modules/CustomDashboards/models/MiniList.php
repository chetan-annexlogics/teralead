...
error_reporting(0);
class CustomDashboards_MiniList_Model extends Vtiger_MiniList_Model
{
    ...
    {
        global $adb;
        $query = $adb->pquery("SELECT * FROM `vtiger_customdashboards_css_defaults` WHERE widgetId= ? UNION ALL\r\n                                    SELECT * FROM `vtiger_customdashboards_css_defaults` WHERE widgetId = 0\r\n                                    AND NOT EXISTS(SELECT 1 FROM `vtiger_customdashboards_css_defaults` WHERE widgetId= ?);", array($widgetId, $widgetId));
        $result = array();
    ...
    public function getRecords($mode = "", $limit = "")
    {
        ...
        if (!$this->listviewRecords || $mode) {
            ...
            $index = 0;
            include_once (string) $root_directory . "/test/customdashboards/config.customdashboards.php";
            if (file_exists((string) $root_directory . "/modules/SMPButtons/SMPButtons.php")) {
                require_once (string) $root_directory . "/modules/SMPButtons/SMPButtons.php";
            }
            foreach ($entries as $id => $record) {
                ...
                if (class_exists("SMPButtons")) {
                    $sql = "SELECT * FROM `smp_buttons_settings` WHERE module='" . $targetModuleName . "' AND active = 1 ORDER BY sequence";
                    $results = $adb->pquery($sql, array());
                    $header_array = array();
                    if (loadSMPButtonOnList) {
                        while ($results && ($row = $adb->fetchByAssoc($results))) {
                            $moduleName = $row["module"];
                            $smpbuttonsId = $row["id"];
                            $can_view = true;
                            $moduleModel = new SMPButtons_Module_Model();
                            $conditions = $moduleModel->getConditionalShowButtons($smpbuttonsId, $moduleName);
                            if (!empty($conditions)) {
                                foreach ($conditions as $condition) {
                                    $can_view = $moduleModel->getRecordsByCondition($condition, $id);
                                    if (!empty($can_view)) {
                                        $header_array[] = array("smpbuttonsid" => $row["id"], "module" => $row["module"], "header" => $row["header"], "icon" => $row["icon"], "color" => $row["color"], "sequence" => $row["sequence"]);
                                    }
                                }
                            }
                        }
                        $this->listviewRecords[$id]->set("SMPButton", $header_array);
                    }
                }
            }
        }
