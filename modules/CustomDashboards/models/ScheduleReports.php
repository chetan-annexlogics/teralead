...
class CustomDashboards_ScheduleReports_Model extends Vtiger_Base_Model
{
    ...
    public static function getInstanceById($recordId)
    {
        ...
        if (!empty($recordId)) {
            $scheduledReportResult = $db->pquery("SELECT * FROM vtiger_schedulecustomdashboards WHERE reportid = ?", array($recordId));
    ...
    public function saveScheduleReport()
    {
        ...
        if ($isReportScheduled == "0" || $isReportScheduled == "" || $isReportScheduled == false) {
            $deleteScheduledReportSql = "DELETE FROM vtiger_schedulecustomdashboards WHERE reportid=?";
            $adb->pquery($deleteScheduledReportSql, array($reportid));
        } else {
            $checkScheduledResult = $adb->pquery("SELECT next_trigger_time FROM vtiger_schedulecustomdashboards WHERE reportid=?", array($reportid));
            if (0 < $adb->num_rows($checkScheduledResult)) {
                $scheduledReportSql = "UPDATE vtiger_schedulecustomdashboards SET scheduleid=?, recipients=?, schdate=?, schtime=?, schdayoftheweek=?, schdayofthemonth=?, schannualdates=?, specificemails=?, from_address=?, subject=?, body=?, next_trigger_time=?, fileformat = ?, signature = ?, signature_user =? WHERE reportid=?";
                ...
                $scheduleReportSql = "INSERT INTO vtiger_schedulecustomdashboards (reportid,scheduleid,recipients,schdate,schtime,schdayoftheweek,schdayofthemonth,schannualdates,next_trigger_time,specificemails,from_address,subject,body,fileformat,signature,signature_user) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public function sendEmail()
    {
        ...
        vimport("~modules/CustomDashboard/models/Record.php");
        $reportRecordModel = CustomDashboards_Record_Model::getInstanceById($this->get("reportid"));
        $currentTime = date("Y-m-d H:i:s");
        ...
        $vtigerMailer->Sender = $this->get("from_address");
        $vtigerMailer->From = $this->get("from_address");
        ...
        $oReportRun = CustomDashboardRun::getInstance($this->get("reportid"));
        ...
        $queryInsertCustomDashboardHistorySendMail = "INSERT INTO vtiger_customdashboardshistory(`reportid`,`reportname`,`date_sent`,`from_email`,`to_email`,`cc_email`,`bcc_email`,`email_subject`,`email_body`,`result`) VALUES(?,?,?,?,?,?,?,?,?,?)";
        ...
        $db->pquery($queryInsertCustomDashboardHistorySendMail, $params);
        if ($reportType != "chart") {
            foreach ($attachments as $attachmentName => $path) {
                unlink($path);
            }
    ...
    public function updateNextTriggerTime()
    {
        ...
        $adb->pquery("UPDATE vtiger_schedulecustomdashboards SET next_trigger_time=? WHERE reportid=?", array($nextTriggerTime, $this->get("reportid")));
        Vtiger_Utils::ModuleLog("ScheduleReprot", "Next Trigger Time updated");
    }
    public static function getScheduledCustomDashboards()
    {
        $adb = PearDatabase::getInstance();
        $currentTimestamp = date("Y-m-d H:i:s");
        $result = $adb->pquery("SELECT reportid FROM vtiger_schedulecustomdashboards\r\n\t\t\t\t\t\t\t\tINNER JOIN vtiger_customdashboardmodules ON vtiger_customdashboardmodules.reportmodulesid = vtiger_schedulecustomdashboards.reportid\r\n\t\t\t\t\t\t\t\tINNER JOIN vtiger_tab ON vtiger_tab.name = vtiger_customdashboardmodules.primarymodule AND presence = 0\r\n\t\t\t\t\t\t\t\tWHERE next_trigger_time <= ? AND next_trigger_time IS NOT NULL", array($currentTimestamp));
    ...
    public static function runScheduledCustomDashboards()
    {
        ...
        if (empty($currentModule)) {
            $currentModule = "CustomDashboards";
        }
        if (empty($current_language)) {
            $current_language = "en_us";
        }
        $scheduledReports = self::getScheduledCustomDashboards();
        foreach ($scheduledReports as $reportId => $scheduledReport) {
            $reportRecordModel = CustomDashboards_Record_Model::getInstanceById($reportId);
            $reportType = $reportRecordModel->get("reporttype");
            if ($reportType == "chart") {
                $status = $scheduledReport->sendEmail();
            } else {
                $query = $reportRecordModel->getCustomDashboardSQL("", "PDF");
                $countQuery = $reportRecordModel->generateCountQuery($query);
                if (0 < $reportRecordModel->getCustomDashboardsCount($countQuery)) {
                    $status = $scheduledReport->sendEmail();
                }
            }
            Vtiger_Utils::ModuleLog("ScheduleCustomDashboard Send Mail Status ", $status);
