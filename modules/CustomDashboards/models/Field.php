...
class CustomDashboards_Field_Model extends Vtiger_Field_Model
{
    public static function getPicklistValueByField($fieldName)
    {
        $picklistValues = false;
        if ($fieldName == "reporttype") {
            $picklistValues = array("tabular" => vtranslate("tabular", "CustomDashboards"), "chart" => vtranslate("chart", "CustomDashboards"), "pivot" => vtranslate("pivot", "CustomDashboards"), "sql" => vtranslate("sqlreport", "CustomDashboards"));
        } else {
            if ($fieldName == "foldername") {
                $allFolders = CustomDashboards_Folder_Model::getAll();
                foreach ($allFolders as $folder) {
                    $picklistValues[$folder->get("folderid")] = vtranslate($folder->get("foldername"), "CustomDashboards");
                }
            } else {
                if ($fieldName == "owner") {
                    ...
                    if ($fieldName == "primarymodule") {
                        $reportModel = CustomDashboards_Record_Model::getCleanInstance();
    ...
    public static function getListViewFieldsInfo()
    {
        ...
        foreach ($fields as $field) {
            $fieldsInfo[$field] = CustomDashboards_Field_Model::getFieldInfoByField($field);
        }
