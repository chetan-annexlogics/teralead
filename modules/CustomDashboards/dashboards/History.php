...
class CustomDashboards_History_Dashboard extends Vtiger_History_Dashboard
{
    public function process(Vtiger_Request $request, $oldWidget = NULL)
    {
        ...
        if (!$linkId) {
            $linkId = CustomDashboards_Widget_Model::getLinkId($moduleName, $componentName);
        }
        ...
        $widget = CustomDashboards_Widget_Model::getHistoryWidget($linkId, $currentUser->getId(), $request->get("widgetid"));
        if (!$widget->get("id")) {
            $widget = $oldWidget;
        }
        if (!$historyType && !$type) {
        ...
        $historyModel = new CustomDashboards_History_Model();
