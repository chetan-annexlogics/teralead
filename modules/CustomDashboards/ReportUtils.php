 ...
 */
function getFieldByCustomDashboardLabel($module, $label, $mode = "label")
{
...
function isReferenceUITypeCustomDashboard($uitype)
{
...
function IsDateFieldCustomDashboard($reportColDetails)
{
 ...
 * @return String
 */
function getCustomDashboardFieldValue($report, $picklistArray, $dbField, $valueArray, $fieldName, $operation = false)
{
    global $current_user;
...
function transformAdvFilterListToDBFormatCustomDashboard($advFilterList)
{
    $db = PearDatabase::getInstance();
...
function getCustomDashboardSearchCondition($searchParams, $filterId)
{
    if (!empty($searchParams)) {
        ...
        foreach ($searchParams as $i => $condition) {
            ...
            if ($fieldName == "reportname" || $fieldName == "description") {
                $conditionQuery .= " vtiger_customdashboard." . $fieldName . " LIKE ? ";
                ...
                if ($fieldName == "reporttype" || $fieldName == "foldername" || $fieldName == "owner") {
                    ...
                    $conditionQuery .= " vtiger_customdashboard." . $fieldName . " IN (" . generateQuestionMarks($searchValue) . ") ";
                    ...
                    if ($fieldName == "primarymodule") {
                        $searchValue = explode(",", $searchValue);
                        $conditionQuery .= " vtiger_customdashboardmodules." . $fieldName . " IN (" . generateQuestionMarks($searchValue) . ") ";
                        foreach ($searchValue as $value) {
                            array_push($params, $value);
                        }
                    }
