...
foreach ($reports as $key => $report) {
    ...
    insertCustomDashboards($queryid, $reportid[$report["reportfolder"]], $report["reportname"], $report["description"], $report["reporttype"]);
    insertSelectColumns($queryid, $selectcolumns[$key]);
    insertCustomDashboardModules($queryid, $reportmodules[$key]["primarymodule"], $reportmodules[$key]["secondarymodule"]);
...
$adb->pquery("UPDATE vtiger_customdashboard SET sharingtype='Public'", array());
...
{
    global $adb;
    $sql = "INSERT INTO vtiger_customdashboardfolder (FOLDERNAME,DESCRIPTION,STATE) VALUES(?,?,?)";
...
/** Function to store the vtiger_field names selected for a vtiger_customdashboard to a database
 *
 *
 */
...
/** Function to store the vtiger_customdashboard details to database
 *  This function accepts queryid,folderid,reportname,description,reporttype
 *  as arguments and store the informations in vtiger_customdashboard vtiger_table
 */
function insertCustomDashboards($queryid, $folderid, $reportname, $description, $reporttype)
{
    global $adb;
    if ($queryid != "") {
        $ireportsql = "insert into vtiger_customdashboard (REPORTID,FOLDERID,REPORTNAME,DESCRIPTION,REPORTTYPE,QUERYID,STATE) values (?,?,?,?,?,?,?)";
...
/** Function to store the vtiger_customdashboard modules to database
 *  This function accepts queryid,primary module and secondary module
 *  as arguments and store the informations in vtiger_customdashboardmodules vtiger_table
 */
function insertCustomDashboardModules($queryid, $primarymodule, $secondarymodule)
{
    global $adb;
    if ($queryid != "") {
        $ireportmodulesql = "insert into vtiger_customdashboardmodules (REPORTMODULESID,PRIMARYMODULE,SECONDARYMODULES) values (?,?,?)";
        $ireportmoduleresult = $adb->pquery($ireportmodulesql, array($queryid, $primarymodule, $secondarymodule));
    }
}
/** Function to store the vtiger_customdashboard sortorder to database
 *  This function accepts queryid,sortlists
 *  as arguments and store the informations sort columns and
 *  and sortorder in vtiger_customdashboardsortcol vtiger_table
 */
function insertSortColumns($queryid, $sortlists)
{
    ...
    if ($queryid != "") {
        foreach ($sortlists as $i => $sort) {
            $sort_bysql = "insert into vtiger_customdashboardsortcol (SORTCOLID,REPORTID,COLUMNNAME,SORTORDER) values (?,?,?,?)";
            $sort_byresult = $adb->pquery($sort_bysql, array($i + 1, $queryid, $sort["columnname"], $sort["sortorder"]));
        }
    }
}
/** Function to store the vtiger_customdashboard sort date details to database
 *  This function accepts queryid,filtercolumn,datefilter,startdate,enddate
 *  as arguments and store the informations in vtiger_customdashboarddatefilter vtiger_table
 */
function insertStdFilter($queryid, $filtercolumn, $datefilter, $startdate, $enddate)
{
    global $adb;
    if ($queryid != "") {
        $ireportmodulesql = "insert into vtiger_customdashboarddatefilter (DATEFILTERID,DATECOLUMNNAME,DATEFILTER,STARTDATE,ENDDATE) values (?,?,?,?,?)";
        $ireportmoduleresult = $adb->pquery($ireportmodulesql, array($queryid, $filtercolumn, $datefilter, $startdate, $enddate));
    }
}
/** Function to store the vtiger_customdashboard conditions to database
