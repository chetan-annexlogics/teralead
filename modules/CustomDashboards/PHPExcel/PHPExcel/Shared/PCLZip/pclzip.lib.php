...
class PclZip
{
    ...
    public $error_code = 1;
    public $error_string = "";
    public $magic_quotes_status = NULL;
                    ...
                    for ($j = 0; $j < sizeof($v_work_list); $j++) {
                        ...
                        if ($v_size_item_list == 1) {
                            ...
                            if ($v_size_item_list == 2) {
                                list($v_result_list[$p_options_list[$i]][$j]["start"], $v_result_list[$p_options_list[$i]][$j]["end"]) = $v_item_list;
                            } else {
    ...
    public function privAdd($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        $i = 0;
        for ($v_count = 0; $i < sizeof($v_header_list); $i++) {
    ...
    public function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        if ($p_entry["status"] == "ok" && ($p_entry["external"] & 16) != 16) {
    ...
    public function privReadEndCentralDir(&$p_central_dir)
    {
        ...
        if (!$v_found) {
            ...
            $v_pos = ftell($this->zip_fd);
            for ($v_bytes = 0; $v_pos < $v_size; $v_pos++) {
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    $i = 0;
    for ($j = 0; $i < $v_list_dir_size && $j < $v_list_path_size && $v_result; $j++) {
        if ($v_list_dir[$i] == "") {
            $i++;
            continue;
        }
