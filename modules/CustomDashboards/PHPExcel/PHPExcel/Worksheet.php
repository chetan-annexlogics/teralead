...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
     ...
     * @var PHPExcel_Worksheet_RowDimension
     */
    private $defaultRowDimension = NULL;
    /**
     ...
     * @var string
     */
    private $sheetState = NULL;
    /**
     * Page setup
     *
     * @var PHPExcel_Worksheet_PageSetup
     */
    private $pageSetup = NULL;
    /**
     * Page margins
     *
        ...
        if (!empty($autoSizes)) {
            ...
            foreach ($this->getCellCollection(false) as $cellID) {
                ...
                if ($cell !== NULL && isset($autoSizes[$this->cellCollection->getCurrentColumn()]) && !isset($isMergeCell[$this->cellCollection->getCurrentAddress()])) {
                    ...
                    $autoSizes[$this->cellCollection->getCurrentColumn()] = max((double) $autoSizes[$this->cellCollection->getCurrentColumn()], (double) PHPExcel_Shared_Font::calculateColumnWidth($this->getParent()->getCellXfByIndex($cell->getXfIndex())->getFont(), $cellValue, $this->getParent()->getCellXfByIndex($cell->getXfIndex())->getAlignment()->getTextRotation(), $this->getDefaultStyle()->getFont()));
                }
            }
        ...
        if ($this->parent && $this->parent->sheetNameExists($pValue)) {
            ...
            while ($this->parent->sheetNameExists($pValue . " " . $i)) {
                ...
                if ($i == 10) {
                    ...
                    if ($i == 100 && 27 < PHPExcel_Shared_String::CountCharacters($pValue)) {
    ...
    {
        if ($this->dirty) {
            $this->hash = md5($this->title . $this->autoFilter . ($this->protection->isProtectionEnabled() ? "t" : "f") . "PHPExcel_Worksheet");
            $this->dirty = false;
        }
        return $this->hash;
    }
            ...
            if (is_object($val) || is_array($val)) {
                if ($key == "cellCollection") {
                    ...
                    if ($key == "drawingCollection") {
                        ...
                        if ($key == "autoFilter" && $this->autoFilter instanceof PHPExcel_Worksheet_AutoFilter) {
