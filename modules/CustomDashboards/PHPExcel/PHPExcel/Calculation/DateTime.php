...
class PHPExcel_Calculation_DateTime
{
            ...
            switch ($method) {
                ...
                case 1:
                    ...
                    if ($years == 1) {
                        ...
                        if ($years == 2) {
                            if ($leapDays == 0 && self::isLeapYear($startYear) && 365 < $days) {
                                $leapDays = 1;
                            } else {
                                if ($days < 366) {
                                    $years = 1;
    ...
    public static function NETWORKDAYS($startDate, $endDate)
    {
        ...
        foreach ($dateArgs as $holidayDate) {
            ...
            if ($startDate <= $holidayDate && $holidayDate <= $endDate && self::DAYOFWEEK($holidayDate, 2) < 6 && !in_array($holidayDate, $holidayCountedArray)) {
                $partWeekDays--;
    ...
    public static function WORKDAY($startDate, $endDays)
    {
        ...
        if (!empty($dateArgs)) {
            ...
            foreach ($holidayDates as $holidayDate) {
                if ($decrementing) {
                    if ($holidayDate <= $startDate && $endDate <= $holidayDate && !in_array($holidayDate, $holidayCountedArray)) {
                        $endDate--;
                        $holidayCountedArray[] = $holidayDate;
                    }
                } else {
                    if ($startDate <= $holidayDate && $holidayDate <= $endDate && !in_array($holidayDate, $holidayCountedArray)) {
                        $endDate++;
                        $holidayCountedArray[] = $holidayDate;
                    }
                }
