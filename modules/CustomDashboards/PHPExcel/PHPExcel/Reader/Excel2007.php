...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    $xmlWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, (string) $rel["Target"]), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions()));
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        foreach ($rels->Relationship as $rel) {
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                ...
                $relsWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, (string) $dir . "/_rels/" . basename($rel["Target"]) . ".rels")), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $xml = new XMLReader();
                        $res = $xml->xml($this->securityScanFile("zip://" . PHPExcel_Shared_File::realpath($pFilename) . "#" . (string) $dir . "/" . $fileWorksheet), NULL, PHPExcel_Settings::getLibXmlLoaderOptions());
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties":
                    $xmlCore = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, (string) $rel["Target"])), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    $xmlStrings = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, (string) $dir . "/" . $xpath["Target"])), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                    ...
                    $xmlStyles = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, (string) $dir . "/" . $xpath["Target"])), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                    $numFmts = NULL;
                    ...
                    if ($xmlWorkbook->sheets) {
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            $fileWorksheet = $worksheets[(string) self::getArrayItem($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];
                            $xmlSheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, (string) $dir . "/" . $fileWorksheet)), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                            ...
                            $hyperlinks = array();
                            if (!$this->readDataOnly) {
                                if ($zip->locateName(dirname((string) $dir . "/" . $fileWorksheet) . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                    $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname((string) $dir . "/" . $fileWorksheet) . "/_rels/" . basename($fileWorksheet) . ".rels")), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                foreach ($comments as $relName => $relPath) {
                                    $relPath = PHPExcel_Shared_File::realpath(dirname((string) $dir . "/" . $fileWorksheet) . "/" . $relPath);
                                ...
                                if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->readDataOnly && $zip->locateName(dirname((string) $dir . "/" . $fileWorksheet) . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                    $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname((string) $dir . "/" . $fileWorksheet) . "/_rels/" . basename($fileWorksheet) . ".rels")), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                                    $vmlRelationship = "";
                                    foreach ($relsWorksheet->Relationship as $ele) {
                                        if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing") {
                                            $vmlRelationship = self::dirAdd((string) $dir . "/" . $fileWorksheet, $ele["Target"]);
                                        }
                                    }
                                    if ($vmlRelationship != "") {
                                        $relsVML = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname($vmlRelationship) . "/_rels/" . basename($vmlRelationship) . ".rels")), "SimpleXMLElement", PHPExcel_Settings::getLibXmlLoaderOptions());
                                        $drawings = array();
                            ...
                            if ($zip->locateName(dirname((string) $dir . "/" . $fileWorksheet) . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                ...
                                foreach ($relsWorksheet->Relationship as $ele) {
                                    if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing") {
                                        $drawings[(string) $ele["Id"]] = self::dirAdd((string) $dir . "/" . $fileWorksheet, $ele["Target"]);
                                    }
                                }
                                        ...
                                        if ($relsDrawing && $relsDrawing->Relationship) {
                                            foreach ($relsDrawing->Relationship as $ele) {
                                                if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image") {
                                                    ...
                                                    if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart" && $this->includeCharts) {
                                        ...
                                        if ($xmlDrawing->twoCellAnchor) {
                                            foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
                                                if ($twoCellAnchor->pic->blipFill) {
                                                    ...
                                                    if ($this->includeCharts && $twoCellAnchor->graphicFrame) {
    ...
    private static function readColor($color, $background = false)
    {
        ...
        if (isset($color["theme"]) && self::$theme !== NULL) {
    ...
    private static function readStyle($docStyle, $style)
    {
        ...
        if (isset($style->alignment)) {
            ...
            $docStyle->getAlignment()->setReadorder(0 < intval((string) $style->alignment["readingOrder"]) ? intval((string) $style->alignment["readingOrder"]) : 0);
        }
