...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
     ...
     * @var array
     */
    protected static $builtInFormats = NULL;
    /**
     * Excel built-in number formats (flipped, for faster lookups)
     *
     * @var array
     */
    protected static $flippedBuiltInFormats = NULL;
    /**
     * Format Code
     *
     * @var string
    ...
    public function getHashCode()
    {
        ...
        return md5($this->formatCode . $this->builtInFormatCode . "PHPExcel_Style_NumberFormat");
    }
    ...
    private static function formatAsFraction(&$value, &$format)
    {
        ...
        if (strpos($format, "0") !== false || strpos($format, "#") !== false || substr($format, 0, 3) == "? ?") {
            ...
            $value = (string) $sign . $integerPart . " " . $adjustedDecimalPart . "/" . $adjustedDecimalDivisor;
        } else {
            $adjustedDecimalPart += $integerPart * $adjustedDecimalDivisor;
            $value = (string) $sign . $adjustedDecimalPart . "/" . $adjustedDecimalDivisor;
        }
    }
            ...
            if (preg_match("/%\$/", $format)) {
                ...
                if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
                    ...
                    if (preg_match("/\\[\\\$(.*)\\]/u", $format, $m)) {
                        list($currencyFormat, $currencyCode) = $m;
