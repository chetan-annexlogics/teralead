...
class PHPExcel_Style_Alignment extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    {
        if (0 < $pValue && $this->getHorizontal() != self::HORIZONTAL_GENERAL && $this->getHorizontal() != self::HORIZONTAL_LEFT && $this->getHorizontal() != self::HORIZONTAL_RIGHT) {
            $pValue = 0;
        }
        if ($this->isSupervisor) {
    ...
    public function getHashCode()
    {
        ...
        return md5($this->horizontal . $this->vertical . $this->textRotation . ($this->wrapText ? "t" : "f") . ($this->shrinkToFit ? "t" : "f") . $this->indent . $this->readorder . "PHPExcel_Style_Alignment");
    }
}
