...
require_once "modules/CustomDashboards/ReportRun.php";
require_once "include/Zend/Json.php";
class VTScheduledCustomDashboard extends CustomDashboards
{
    public $db = NULL;
    public $user = NULL;
    ...
    public function getCustomDashboardScheduleInfo()
    {
        global $adb;
        if (!empty($this->id)) {
            ...
            if ($cachedInfo == false) {
                ...
                if (0 < $adb->num_rows($result)) {
                    ...
                    VTCacheUtils::updateCustomDashboard_ScheduledInfo($this->user->id, $this->id, true, $reportScheduleInfo["format"], $scheduledInterval, $scheduledRecipients, $reportScheduleInfo["next_trigger_time"]);
                    $cachedInfo = VTCacheUtils::lookupReport_ScheduledInfo($this->user->id, $this->id);
                }
            }
            if ($cachedInfo) {
    ...
    public function sendEmail()
    {
        ...
        $oCustomDashboardRun = ReportRun::getInstance($this->id);
    ...
    public static function getScheduledCustomDashboards($adb, $user)
    {
        $currentTime = date("Y-m-d H:i:s");
        ...
        for ($i = 0; $i < $noOfScheduledReports; $i++) {
            ...
            $scheduledInterval = !empty($reportScheduleInfo["schedule"]) ? Zend_Json::decode($reportScheduleInfo["schedule"]) : array();
            $scheduledRecipients = !empty($reportScheduleInfo["recipients"]) ? Zend_Json::decode($reportScheduleInfo["recipients"]) : array();
            $vtScheduledReport = new VTScheduledCustomDashboard($adb, $user, $reportScheduleInfo["reportid"]);
    ...
    public static function runScheduledCustomDashboards($adb)
    {
        ...
        $scheduledReports = self::getScheduledCustomDashboards($adb, $adminUser);
