...
global $mod_strings;
$current_module_strings = return_module_language($current_language, "CustomDashboards");
...
function getVisibleCriteria($recordid = "")
{
    ...
    $selcriteria = "";
    if ($recordid != "") {
        $result = $adb->pquery("select sharingtype from vtiger_customdashboard where reportid=?", array($recordid));
        $selcriteria = $adb->query_result($result, 0, "sharingtype");
    }
    if ($selcriteria == "") {
        $selcriteria = "Public";
    }
    $filter_result = $adb->query("select * from vtiger_customdashboardfilters");
...
{
    global $adb;
    $member_query = $adb->pquery("SELECT vtiger_customdashboardsharing.setype,vtiger_users.id,vtiger_users.user_name FROM vtiger_customdashboardsharing INNER JOIN vtiger_users on vtiger_users.id = vtiger_customdashboardsharing.shareid WHERE vtiger_customdashboardsharing.setype='users' AND vtiger_customdashboardsharing.reportid = ?", array($recordid));
    ...
    $member_query = $adb->pquery("SELECT vtiger_customdashboardsharing.setype,vtiger_groups.groupid,vtiger_groups.groupname FROM vtiger_customdashboardsharing INNER JOIN vtiger_groups on vtiger_groups.groupid = vtiger_customdashboardsharing.shareid WHERE vtiger_customdashboardsharing.setype='groups' AND vtiger_customdashboardsharing.reportid = ?", array($recordid));
    $noofrows = $adb->num_rows($member_query);
    if (0 < $noofrows) {
        for ($i = 0; $i < $noofrows; $i++) {
